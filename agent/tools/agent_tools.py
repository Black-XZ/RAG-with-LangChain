...
def _get_public_ip() -> str:
    ip_sources = agent_conf.get("public_ip_sources", ["https://ipv4.icanhazip.com"])
...
def _gaode_get(path: str, params: dict) -> dict:
    ...
    if not gaode_key or gaode_key == "你的高德key":
        raise ValueError("agent.yml中未配置有效的gaodekey")
...
def fetch_external_data(user_id: str, month: str) -> str:
    ...
    try:
        ...
        # 确保输出为字符串，避免下游 LLM 接口收到 dict 触发 contents 类型错误
